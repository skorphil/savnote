...
export class RecordDraft {
	...
	static resume() {
		...
		if (
			...
			return null;
		return RecordDraft.instance ? RecordDraft.instance : new RecordDraft();
	}

	static delete() {
		RecordDraft.store.delTables();
		RecordDraft.instance = undefined;
	}
	...
	async getRecordData(): Promise<RecordsSchema> {
		...
		for (const asset of Object.values(assets)) {
			...
			void [isDirty, isNew];
			if (isDeleted || institutionDeleted) continue;
	...
	async getQuotes(
		...
		const quotesDataPromises = baseCurrencies.map(async (baseCurrency) => {
			...
			void _;
			for (const counterCurrency of counterCurrencies) {
